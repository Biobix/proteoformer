...
################
# COMMAND LINE:
# ./gene_distribution.pl --treated untreated (or 'treated') --ens_db /data2/steven/ENS_mmu_82.db (--work_dir getcwd --in_sqlite SQLite/results.db --output_file run_name_genedistribution)
...
my ($tooldir,$work_dir,$cores,$in_sqlite,$output_folder,$treated,$ens_db);
GetOptions(
...
"output_folder:s" => \$output_folder,   # Output folder                                                 optional  argument (default = output)
"treated:s" => \$treated,				# Define data('no':no treatment/CHX or 'yes':treated(LTM/Harr))	mandatory argument
"ens_db:s" => \$ens_db                  # Define Ensembl Database                                       mandatory argument
);
...
if ($ens_db) {
    print "The Ensembl database is  : $ens_db\n";
} else {
    print "ERROR: do not forget to give the Ensembl database!";
    die;
}
...
my $output_file = $out_dir."/".$run_name."_ens".$version."_".$mapper."_".$treated;
my $out_table = $output_file."_genedistribution.txt";
my $out_pdf1 = $output_file."_rankedgenes.png";
my $out_pdf2 = $output_file."_cumulative.png";
my $out_pdf3 = $output_file."_density.png";
