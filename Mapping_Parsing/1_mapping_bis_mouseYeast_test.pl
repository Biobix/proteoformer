...
my $STAR_ref_loc = ($clusterPhoenix eq "Y") ? $HOME."/STARIndex/" : $IGENOMES_ROOT."/".$spec."/Ensembl/".$assembly."/Sequence/STARIndexMmuYeast/"; #Differs when yeast sequences are included
...
sub RIBO_parse_store {
    ...
    # Store statistics from R_TMP file in database
    foreach my $chr(keys %chr_sizes){
        R_temp($chr,$seqFileName);
        
    }

    #remove R_tmp file
    system("rm -rf ".$TMP."/RTMP");
    ...
    if($species eq 'fruitfly'){
        ...
        foreach my $chr (keys %chr_sizes){
            ...
            #Get data from DB
            $query = "SELECT length,reads from R_temp where chr = '".$chr."_".$seqFileName."'";
            $sth = $dbh_sqlite_results->prepare($query);
            $sth->execute();
    ...
    $dbh_sqlite_results->do($query);
    $query = "DROP table if exists R_temp";
    $dbh_sqlite_results->do($query);
...
sub RIBO_parsing_genomic_per_chr {
    ...
    system("mkdir -p ".$TMP."/RTMP/");
    open (RTMP,">".$TMP."/RTMP/R_TMP_".$seqFileName."_".$chr.".txt") || die "ERROR opening R_tmp file to write \n";
...
sub R_temp{
    ...
    #Read statistics from R_TMP file
    open (RTMPin,"<".$TMP."/RTMP/R_TMP_".$seqFileName."_".$chr.".txt") || die "ERROR reading R_tmp file \n";
    my $read = <RTMPin>;
    my @stats=split("\t",$read);
    my $read_mapped_length=$stats[0];
    my $nr_reads=$stats[1];
    close(RTMPin);
    ...
    my $query_table = "CREATE TABLE IF NOT EXISTS `R_temp` (
    `chr` char(50) NOT NULL default '',
    ...
    #Add to table
    my $query = "INSERT INTO R_temp (chr,length,reads) VALUES (\'".$chr."_".$seqFileName."\',\'".$read_mapped_length."\',\'".$nr_reads."\')";
    $dbh->do($query);
