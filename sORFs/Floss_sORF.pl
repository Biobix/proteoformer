...
# ./Floss_sORF.pl --sqlite SQLite/results.db  --tis_ids 1
## Galaxy ##
...
#           GOED VB
#           select chr,strand, start, RPF, count from count_fastq1_splitRPF where chr='3' AND strand='1' AND start>95428560 AND start<95428829;
#           sorfID=15650
...
foreach my $analysis_id (@$idsref) {
    ...
    foreach my $chr (sort keys %{$chrs}){
        ...
        my $LD = makeLD($dsn_results, $us_results, $pw_results,$chr, $fastqname, $analysis_id);
    ...
    my $create_table = "CREATE TABLE `TIS_sORFs_".$analysis_id."_transcripts_FLOSS` (`sorfID` INTEGER PRIMARY KEY, `nreads` INTEGER, `FLOSS` DECIMAL(9,7),`classification` VARCHAR(15));";
    $dbh->do($create_table);
    
    #Store data in csv file
    print "export FLOSS data of sORFs to csv file \n";
    open sorfFLOSS, ">>".$TMP."/sorfFLOSS.csv" or die "ERROR exporting FLOSS data";
    foreach my $sorfID (keys %{$classTable}){
        if(defined $classTable->{$sorfID}->{"FLOSS"}){
            print sorfFLOSS $sorfID.",".$classTable->{$sorfID}->{"nreads"}.",".$classTable->{$sorfID}->{"FLOSS"}.",".$classTable->{$sorfID}->{"Class"}."\n";
        } else {
            print sorfFLOSS $sorfID.",".$classTable->{$sorfID}->{"nreads"}.",,".$classTable->{$sorfID}->{"Class"}."\n";
        }
    }
    close(sorfFLOSS);
    
    #Fill final table out of csv file
    print "Insert FLOSS data from csv file into table TIS_sORFs_".$analysis_id."_transcripts_FLOSS\n";
    system("sqlite3 -separator , ".$db_results." \".import ".$TMP."/sorfFLOSS.csv TIS_sORFs_".$analysis_id."_transcripts_FLOSS\"")== 0 or die "system failed: $?";
    
    #Remove csv file
    system("rm ".$TMP."/sorfFLOSS.csv");
...
sub makeLD{
    ...
    #Features from the transcript table
    my @features = ("biotype","chr","strand","sorf_begin","sorf_end","annotation");
    
    #get features from transcript table
    my $sorfQuery = "select sorf_id, biotype, chr, strand, sorf_begin, sorf_end, annotation from TIS_sorfs_".$analysisID."_transcripts where chr='".$chr."';";
    my $sth = $dbh->prepare($sorfQuery);
    $sth->execute();
    while (my @transcriptInfo= $sth->fetchrow_array) {
        #Put all features in hash
        for (my $i=0; $i<scalar(@transcriptInfo)-1;$i++){
            $sorfLD->{$transcriptInfo[0]}->{$features[$i]} = $transcriptInfo[$i+1];
        }
        
        for (my $startRPF = $transcriptInfo[4]; $startRPF<=$transcriptInfo[5]; $startRPF++){
            for (my $RPFlength = 26; $RPFlength<=34; $RPFlength++){
                my $RPFid = $RPFlength.$startRPF;
                if($RPFs->{$RPFid}->{'count'}){
                    if($RPFs->{$RPFid}->{'strand'} == $transcriptInfo[3]){
                        if ($sorfLD->{$transcriptInfo[0]}->{"RPF"}->{$RPFlength}){
                            $sorfLD->{$transcriptInfo[0]}->{"RPF"}->{$RPFlength}=$sorfLD->{$transcriptInfo[0]}->{"RPF"}->{$RPFlength}+$RPFs->{$RPFid}->{'count'};
                        } else {
                            $sorfLD->{$transcriptInfo[0]}->{"RPF"}->{$RPFlength}=$RPFs->{$RPFid}->{'count'};
                        }
                        if($sorfLD->{$transcriptInfo[0]}->{"nreads"}){
                            $sorfLD->{$transcriptInfo[0]}->{"nreads"} = $sorfLD->{$transcriptInfo[0]}->{"nreads"} + $RPFs->{$RPFid}->{'count'};
                        } else {
                            $sorfLD->{$transcriptInfo[0]}->{"nreads"} = $RPFs->{$RPFid}->{'count'};
                        }
                    }
                }
            }
...
sub classify {
    ...
    foreach my $sorfID (keys %{$LD}){
        if (defined $LD->{$sorfID}->{"nreads"} && $LD->{$sorfID}->{"nreads"}>0){
            my $nreads = $LD->{$sorfID}->{"nreads"};
            $sorfScore->{$sorfID}->{"FLOSS"} = flossCalc($sorfID, $LD, $refFracs);
            if ($nreads<$smoother->{"min"}){
                $sorfScore->{$sorfID}->{"Class"} = "Not in cutoff range";
                ...
                if ($cutoffBorder eq "NA"){
                    next;
                } elsif ($sorfScore->{$sorfID}->{"FLOSS"}<$cutoffBorder){
                    $sorfScore->{$sorfID}->{"Class"} = "Good"; #Means probably coding
                } else {
                    $sorfScore->{$sorfID}->{"Class"} = "Extreme"; #Means probably non-coding
                }
            }
        } else {
            $LD->{$sorfID}->{"nreads"} = 0;
            my $nreads = $LD->{$sorfID}->{"nreads"};
            $sorfScore->{$sorfID}->{"FLOSS"} = undef;
            $sorfScore->{$sorfID}->{"Class"} = "No reads";
        }
    }
    ...
    foreach my $sorfID (keys %{$sorfScore}){
        if (defined $sorfScore->{$sorfID}->{"FLOSS"}){
            print classificationFH $sorfID."\t".$LD->{$sorfID}->{"nreads"}."\t".$sorfScore->{$sorfID}->{"FLOSS"}."\t".$sorfScore->{$sorfID}->{"Class"}."\n";
        } else {
            print classificationFH $sorfID."\t".$LD->{$sorfID}->{"nreads"}."\tundef\t".$sorfScore->{$sorfID}->{"Class"}."\n";
        }
    }
