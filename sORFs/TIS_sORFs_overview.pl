...
# ./TIS_overview.pl --sqlite /home/galaxy/data/SQLite/results.db
...
##################################################################
## Creating TAB separated file based on TIS_sORFs_overview table #
##                                                               #
##################################################################
...
sub create_tab_file{
    ...
    while (my $row = $sth->fetchrow_hashref) {
        print TempTIS $row->{"ID"}."\t".$row->{"local_max"}."\t".$row->{"min_count"}."\t".$row->{"R"}."\n";
    }
