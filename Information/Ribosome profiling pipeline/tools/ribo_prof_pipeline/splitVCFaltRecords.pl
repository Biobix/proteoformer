...
# Input = text file with 4 or 5 columns: chromosome name, position, reference base, alternative base (& allelic frequency)
#
...
while (<IN>){
    ...
    if ($line[3] =~ /,/){ # check whether the 'alt' column contains a comma
        ...
        } elsif ($lineLength == 5){ # there could be an extra column for allelic frequency
            foreach my $altBase (@alt){
                print OUT $line[0].";".$line[1].";".$line[2].";".$altBase.";".$line[4]."\n";
            }
        }
    } else {
        print OUT $_."\n";
