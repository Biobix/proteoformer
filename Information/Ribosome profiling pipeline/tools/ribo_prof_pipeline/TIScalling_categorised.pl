...
# ./TIScalling_categorised.pl --sqlite SQLite/results_STARclip_26-35_1-14MM.db --cores 22 --local_max 1 --R_aTIS 0.01 --min_count_aTIS 5 --R_5 0.05 --min_count_5 10 --R_CDS 0.15 --min_count_CDS 15 --R_3 0.05 --min_count_3 10 --R_ntr 0.05 --min_count_ntr 10
...
my ($ensemblversion,$species,$ens_db,$IGENOMES_ROOT,$cores) = get_arguments($dsn_results,$us_results,$pw_results);
...
# Sqlite Ensembl
#my $db_ENS  = $ens_db_root."/ENS_".$spec_short."_".$ensemblversion.".db";
my $db_ENS  = $ens_db;
...
sub TIS_calling{
    ...
    foreach my $chr (sort keys %{$chrs}){
        ...
        foreach my $tr_id (sort {$a <=> $b} keys %{$trs}){
            ...
            my $TIS = analyze_transcript_peaks($trans,$exons,$tr_seq,$LTM_reads,$CHX_reads,$tr_id,$trs,$local_max,$chr,$min_count_aTIS,$min_count_5,$min_count_CDS,$min_count_3,$min_count_ntr,$R_aTIS,$R_5,$R_CDS,$R_3,$R_ntr);
        ...
        foreach my $key (sort {$a <=> $b} keys %{$peaks}){
            ...
            if($count >= $peaks->{$key}{'min_count'} || ($count < $peaks->{$key}{'min_count'} && $peaks->{$key}{'annotation'} eq "aTIS")){
                ...
                if($CHX_split_total == 0){
                    ...
                    $Rchx = ($CHX_split_total/$total_CHX);
                    $Rchx = $Rchx*10;
                }
                my $R_test = $Rltm - $Rchx;
...
sub get_transcripts_and_reads {
    ...
    # Get transcripts
    #my $query = "SELECT transcript_id,seq_region_id,seq_region_strand,seq_region_start,seq_region_end,read_counts,biotype,stable_id FROM tr_translation WHERE seq_region_id = $seq_region_id AND exon_coverage = 'Yes'";
    ...
    #print "translation data for chromosome: ".$chr." in hash! \n\n";
    ...
    # print "translation and read data for chromosome: ".$chr." in hash! \n\n";
...
sub get_chrs {
    ...
    my $chrs    =   {};
    my $dbh     =   dbh($db,$us,$pw);
    my ($line,@chr,$coord_system_id,$seq_region_id,@ids,@coord_system,$chr);
...
sub get_arguments{
    ...
    return($ensemblversion,$species,$ens_db,$igenomes_root,$nr_of_cores);
