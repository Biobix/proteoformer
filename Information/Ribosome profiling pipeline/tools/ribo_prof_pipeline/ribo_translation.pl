...
# ** example: ./ribo_translation.pl (--work_dir /data/gerbenm/test/ --in_sqlite SQLite/results.db --ens_db SQLite/ens.db --tmp /data/gerbenm/test/SQLite/ --out_sqlite SQLite/results.db)
# GALAXY
...
#my $db_ensembl = "ENS_".$spec_short."_".$version;
...
sub translation_per_chr{
	...
	foreach my $chr(@ch){
		...
		while(my @result2 = $execute2->fetchrow_array()){
			#$result2[0]: gene_id
			#$result2[1]: gene_stable_id
			#$result2[2]: transcript_id
			#$result2[3]: start_transcript
			#$result2[4]: stop_transcript
			#$result2[5]: strand_gene (forward=1,reverse=-1)
			#$result2[6]: transcript_stable_id
			#$result2[7]: first_exon_id
			#$result2[8]: last_exon_id
			#$result2[9]: seq_start (offset position translation start)
			#$result2[10]: seq_end (offset position translation stop)
			#$result2[11]: biotype protein
			
			# Gene info
			$gene_id{$result2[1]} = [$result2[0],$result2[5]];
			$gene_count{$result2[1]} = 0; # Initialize count hash
			#$gene_strand{$result2[1]} = $result2[5];
			...
			#$trans_seq_region{$result2[2]} = [$result2[3],$result2[4]];	
		}
		$execute2->finish();
		...
		while(my @result3 = $execute3->fetchrow_array()){
			#$result3[0]: gene_id
			#$result3[1]: gene_stable_id
			#$result3[2]: transcript_id
			#$result3[3]: start_transcript
			#$result3[4]: stop_transcript
			#$result3[5]: strand_gene (forward=1,reverse=-1)
			#$result3[6]: transcript_stable_id
			#$result3[7]: biotype protein
			
			# Gene info
			$gene_id{$result3[1]} = [$result3[0],$result3[5]];
			$gene_count{$result3[1]} = 0; # Initialize count hash
			#$gene_strand{$result3[1]} = $result3[5];
			...
			#$trans_seq_region{$result3[2]} = [$result3[3],$result3[4]];
		}
		$execute3->finish();
		...
		foreach my $transcript(keys %transcript_noncoding){
			...
			while(my @result4 = $execute4->fetchrow_array()){
				#$result4[0]: exon_id
				#$result4[1]: start_exon
				#$result4[2]: stop_exon
				#$result4[3]: strand_exon (forward=1,reverse=-1)
				#$result4[4]: exon_stable_id
				#$result4[5]: exon_rank
		...
		foreach my $transcript(keys %transcript_coding){
			...
			while(my @result5 = $execute5->fetchrow_array()){
				#$result5[0]: exon_id
				#$result5[1]: start_exon
				#$result5[2]: end_exon
				#$result5[3]: strand_exon (forward=1,reverse=-1)
				#$result5[4]: exon_stable_id
				#$result5[5]: exon_rank
			...
			while(my @result6 = $execute6->fetchrow_array()){
				#$result6[0]: exon_id
				#$result6[1]: start_exon
				#$result6[2]: end_exon
				#$result6[3]: strand_exon (forward=1,reverse=-1)
				#$result6[4]: exon_stable_id
				#$result6[5]: exon_rank
			...
			while(my @result7 = $execute7->fetchrow_array()){
				#$result7[0]: exon_id
				#$result7[1]: exon_start
				#$result7[2]: exon_end
				#$result7[3]: strand (1=forward; -1=reverse)
				#$result7[4]: exon_stable_id;
				#$result7[5]: exon_rank
		...
		while(my @result8 = $execute8->fetchrow_array()){
			#$result8[0]: chromosome
			#$result8[1]: strand (1=forward; -1=reverse)
			#$result8[2]: position ribo_read("A-site")
			#$result8[3]: read_count
