...
"out_table1:s" =>\$out_table1,			# Define output variables galaxy								optional argument (default = made with $output_file)
"out_table2:s" =>\$out_table2,			# Define output variables galaxy								optional argument (default = made with $output_file)
...
# Get ARG vars
my ($species,$version,$run_name,$mapper,$IGENOMES_ROOT,$ens_db) = get_ARG_vars($db_ribo,$user,$pw);
...
if(!defined($output_file)){$output_file = $run_name."_ens".$version."_".$mapper."_".$treated;}
if(!defined($out_table1)){$out_table1 = $out_dir.$output_file."_annotation_coding.txt";}
if(!defined($out_table2)){$out_table2 = $out_dir.$output_file."_annotation_noncoding.txt";}
if(!defined($out_pdf1)){$out_pdf1 = $out_dir.$output_file."_annotation_coding.pdf";}
if(!defined($out_pdf2)){$out_pdf2 = $out_dir.$output_file."_annotation_noncoding.pdf";}
...
sub get_ARG_vars{
	...
	return($species,$version,$run_name,$mapper,$IGENOMES_ROOT,$ens_db);
...
sub piecharts{
	...
	# Execute Rscript
	system("Rscript ".$galaxydir."/tools/ribo_prof_pipeline/metagenic_piecharts.R ".$out_table1." ".$out_table2." ".$out_pdf1." ".$out_pdf2);	
