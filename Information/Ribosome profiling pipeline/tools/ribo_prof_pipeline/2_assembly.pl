...
##############
##Command-line
# ./2_assembly.pl  --sqliteRES sqlite_results_DB --sqliteENS sqlite_ensembl_DB --tis_ids list_of_analysis_ids (--snp NO --dir /data/RIBO_runs/RIBO_Ingolia_GerbenM/ --tmp /data/RIBO_runs/RIBO_Ingolia_GerbenM/tmp/ --localmax 1 --mincount_aTIS 10 --R_aTIS .05 and the other mincount/R values --out_sqlite $out_sqlite)
...
sub construct_trans_prod {
    ...
    my $chrs = get_chrs($dbh_ENS,$chromosome_sizes,$assembly);
    my %chrs = %{$chrs};
    $dbh_ENS->disconnect();
...
#### get chr_name,chr_seq_region_id from Ensembl ###
#sub get_chrs {
#
#    # Catch
#    my $dbh = $_[0];
...
#
#   	# Get chrs with seq_region_id
#	my $query = "SELECT name,seq_region_id FROM seq_region where coord_system_id = '1'";
#	my $sth = $dbh->prepare($query);
#	$sth->execute();
#	$chrs = $sth->fetchall_hashref('name');
#
#	# Return
#	return($chrs);
#
