...
################
# COMMAND LINE:
# ./gene_distribution.pl --igenomes_root IGENOMES_ROOT --treated untreated (or 'treated') (--work_dir getcwd --in_sqlite SQLite/results.db --ens_db SQLite/ens.db --output_file run_name_genedistribution)
# **example: ./gene_distribution.pl --cores 10 --igenomes /data/igenomes/ --treated no (--work_dir /data/RIBO_runs/RIBO_Ingolia_GerbenM/ --in_sqlite SQLite/results.db --ens_db SQLite/ens.db --output_file mESC_GA_ens72_STAR_untreated_genedistribution)
# GALAXY
...
sub make_plots{
	...
	# Execute Rscript
	system("Rscript ".$galaxydir."/tools/ribo_prof_pipeline/quality_plots.R ".$out_table." ".$out_pdf1." ".$out_pdf2." ".$out_pdf3);	
}