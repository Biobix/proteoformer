...
def output_csv(identifications, csv_file):
    ...
    with open(csv_file, 'w') as FW:
        keys = ["base_proteoform","proving_peptides","peptide_PEP","annotations","bin_codes","protein_group"]
...
def determine_identifications(customdb_peptides, protein_groups):
    ...
    for idx, row in customdb_peptides.iterrows():
        ...
        #Save annotations and bin codes
        annotations_string=""
        bin_codes_string=""
        for p in proteinGroupId.split(';'):
            m = re.match('^ENST\d+\_.+\_\d+\_(.+)\_(\d+)db\d+$', p)
            if m:
                if annotations_string=="":
                    annotations_string+=m.group(1)
                    bin_codes_string+=m.group(2)
                else:
                    annotations_string = annotations_string +", " + m.group(1)
                    bin_codes_string = bin_codes_string +", " + m.group(2)
        identifications[proteinGroupId]['annotations'] = annotations_string
        identifications[proteinGroupId]['bin_codes'] = bin_codes_string
