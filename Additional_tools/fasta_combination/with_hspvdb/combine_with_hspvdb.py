...
def main():
    ...
    parser = argparse.ArgumentParser(argument_default=argparse.SUPPRESS,
                                     description="This tool is part of the PROTEOFORMER pipeline. It combines "
                                                 "a (combined) fasta file of PROTEOFORMER with UniProt and HSPVdb, ready for MS inspection.",
                                     add_help=False)
    ...
    man_args.add_argument("--hspv", "-s", action="store", required=True, nargs="?", metavar="PATH",
                          default="", type=str, help="Input HSPVdb converted fasta file (mandatory)")
    ...
    opt_args.add_argument("--output_fasta", "-o", action="store", required=False, nargs="?", metavar="PATH",
                          default="comb_fasta_uniprot_rnaseq.fasta", type=str, help="Output fasta file with combination of "
                                                                                            "data of Uniprot and PROTEOFORMER "
                                                                                            "(default: comb_fasta_uniprot_hspv_rnaseq.fasta)")
    opt_args.add_argument("--overview_file", "-t", action="store", required=False, nargs="?", metavar="PATH",
                            default="uniprot_rnaseq_overview.txt", type=str, help="Path to a file where information about "
                                                                            "the input files and the database combinations are stored "
                                                                            "(default: uniprot_hspv_rnaseq_overview.txt)")
    ...
    if args.output_fasta == "comb_fasta_uniprot_hspv_rnaseq.fasta":
        args.output_fasta = args.workdir + "/" + args.output_fasta
    if args.overview_file == "uniprot_hspv_rnaseq_overview.txt":
    ...
    sources = ["rnaseq", "uniprot", "hspv"]
    inputs=[args.fasta, args.uniprot, args.hspv]
...
def construct_venn(counts, venn_file):
    ...
    if(len(counts.keys())==3):
        subsets = (counts['rnaseq'], counts['uniprot'], counts['uniprot+rnaseq'])
        set_labels = ('1', '2')
        textstr = "1: RNAseq\n2: uniprot"
    ...
    if(len(counts.keys())==7):
        subsets = (counts['rnaseq'], counts['uniprot'], counts['uniprot+rnaseq'], counts['hspv'], counts['hspv+rnaseq'],counts['uniprot+hspv'],counts['uniprot+hspv+rnaseq'])
        set_labels = ('1', '2', '3')
        textstr = "1: RNAseq\n2: UniProt\n3: HSPVdb"
